    ...
    <div class="modal-box pt-4 pb-6 px-6 mt-20 max-h-none w-full max-w-xl">
      ...
      >
        <span class="modal-title">
          {{ t("condition") }} -
          {{ item.name || buildDefaultName(item, template.fields) }}
        </span>
        <a
          href="#"
      ...
      <div>
        ...
        >
          <a
            href="https://www.rackslabs.com/pricing"
            target="_blank"
            class="link"
        ...
        <form @submit.prevent="validateSaveAndClose">
          <div class="my-4">
            ...
            >
              <div v-if="cindex > 0" class="divider -mb-2 mx-1">
                <button
              ...
              <select
                ...
                @change="
                  [
                    ...
                    conditionActions(condition).includes(condition.action)
                      ? ''
                      : (condition.action = conditionActions(condition)[0]),
                  ]
                "
              >
                <option value="" disabled :selected="!condition.field_uuid">
                  {{ t("select_field_") }}
                </option>
                <option
                  v-for="f in fields"
              ...
              <select
                v-if="
                  ['radio', 'select', 'multiple'].includes(
                    conditionField(condition)?.type
                  ) && conditionField(condition)?.options
                "
                class="select select-bordered select-sm w-full bg-white h-11 pl-4 text-base font-normal"
...
export default {
  ...
  data() {
    return {
      conditions: this.item.conditions?.[0]
        ? JSON.parse(JSON.stringify(this.item.conditions))
        : [{}],
    };
  },
  computed: {
      ...
      if (this.item.submitter_uuid) {
        return this.template.fields.reduce((acc, f) => {
          if (
            f !== this.item &&
            (!f.conditions?.length ||
              !f.conditions.find((c) => c.field_uuid === this.item.uuid))
          ) {
            acc.push(f);
          }
