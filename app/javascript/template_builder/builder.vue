  ...
  >
    ...
    >
      ...
      <div class="space-x-3 flex items-center flex-shrink-0">
        ...
        <template v-else>
          ...
          <span v-if="editable" id="save_button_container" class="flex">
            <button
    ...
    >
      ...
      >
        ...
        >
          <div
            class="bg-base-200 rounded-lg p-5 text-center space-y-4 draw-field-container"
          >
            <p>
        ...
        <div>
          <Fields
            ...
            @set-drag-placeholder="
              $refs.dragPlaceholder.dragPlaceholder = $event
            "
            @change-submitter="selectedSubmitter = $event"
            @drag-end="
              [
                (dragField = null),
                ($refs.dragPlaceholder.dragPlaceholder = null),
              ]
            "
    ...
    <div id="docuseal_modal_container" class="modal-container" />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    isMobile() {
      ...
      return (
        isMobileSafariIos || /android|iphone|ipad/i.test(navigator.userAgent)
      );
    },
    defaultDateFormat() {
  ...
  created() {
    ...
    this.defineSubmitters.forEach((name, index) => {
      ...
      if (
        defineSubmittersUuids.filter(Boolean).length ||
        existingSubmittersUuids.filter(Boolean).length
      ) {
        submitter.uuid =
          defineSubmittersUuids[index] ||
          existingSubmittersUuids[index] ||
          submitter.uuid ||
          v4();
      } else {
        submitter.uuid ||= v4();
  ...
  methods: {
    ...
    reorderFields(item) {
      ...
      const attachmentUuids = this.template.schema.map(
        (e) => e.attachment_uuid
      );
      ...
      this.template.fields.forEach((f) => {
        if (f.areas?.length) {
          const firstArea = f.areas.reduce((min, a) => {
            return attachmentUuids.indexOf(a.attachment_uuid) <
              attachmentUuids.indexOf(min.attachment_uuid)
              ? a
              : min;
          }, f.areas[0]);
      ...
      const sortArea = (aArea, bArea) => {
        if (aArea.attachment_uuid === bArea.attachment_uuid) {
          ...
          return (
            attachmentUuids.indexOf(aArea.attachment_uuid) -
            attachmentUuids.indexOf(bArea.attachment_uuid)
          );
        }
      };

      itemFields.sort((aField, bField) => {
        const aArea = (fieldAreasIndex[aField.uuid] ||= [
          ...(aField.areas || []),
        ].sort(sortArea)[0]);
        const bArea = (fieldAreasIndex[bField.uuid] ||= [
          ...(bField.areas || []),
        ].sort(sortArea)[0]);
      ...
      const insertBeforeAttachmentUuids = attachmentUuids.slice(
        this.template.schema.indexOf(item) + 1
      );
      ...
      if (insertBeforeAttachmentUuids.length) {
        const insertAfterField = fields.find((f) => {
          if (f.areas?.length) {
            return f.areas.find((a) =>
              insertBeforeAttachmentUuids.includes(a.attachment_uuid)
            );
          } else {
            return false;
          }
        ...
        if (
          fields.length &&
          itemFields.length &&
          this.template.fields.indexOf(fields[0]) >
            this.template.fields.indexOf(itemFields[0])
        ) {
      ...
      if (!this.fieldsDragFieldRef.value) {
        if (["select", "multiple", "radio"].includes(field.type)) {
          if (this.dragField?.options?.length) {
            field.options = this.dragField.options.map((option) => ({
              value: option,
              uuid: v4(),
            }));
          } else {
      ...
      data.schema.forEach((schemaItem, index) => {
        ...
        if (this.template.schema[index]) {
          this.onDocumentReplace({
            ...
            documents: [
              data.documents.find(
                (doc) => doc.uuid === schemaItem.attachment_uuid
              ),
            ],
          });
        } else {
