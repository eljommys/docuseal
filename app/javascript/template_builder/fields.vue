<template>
  ...
  >
    <Field
      ...
      "
      @dragend="
        [(fieldsDragFieldRef.value = null), $emit('set-drag-placeholder', null)]
      "
      @remove="removeField"
  ...
  >
    <template v-for="(icon, type) in fieldIconsSorted" :key="type">
      <button
        ...
        @dragend="$emit('drag-end')"
        @click="
          ['file', 'payment', 'verification'].includes(type)
            ? $emit('add-field', type)
    ...
    fieldIconsSorted() {
      if (this.fieldTypes.length) {
        ...
        return Object.fromEntries(
          Object.entries(this.fieldIcons).filter(([key]) => key !== "heading")
        );
      }
    },
    submitterFields() {
